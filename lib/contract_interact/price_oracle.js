 ...
 *
 */
PriceOracle.prototype.getPrice = async function(baseCurrency, quoteCurrency){
...
PriceOracle.prototype.decimalPrice = function(baseCurrency, quoteCurrency){
  return (this.getPrice(baseCurrency, quoteCurrency))/(helper.decimalPrecisionInWei());
},
 ...
 *
 */
PriceOracle.prototype.setPrice = async function(baseCurrency, quoteCurrency, price){
    logger.info("Input Price: " + price);

    helper.validatePrice(price);
    const contractAddr = coreAddresses.getAddressOfPriceOracleContract(baseCurrency, quoteCurrency);
    currContract.options.address = contractAddr;
    currContract.setProvider( web3RpcProvider.currentProvider );
 ...
 *
 */
PriceOracle.prototype.getExpirationHeight = async function(baseCurrency, quoteCurrency){
  const contractAddr = coreAddresses.getAddressOfPriceOracleContract(baseCurrency, quoteCurrency);
  currContract.options.address = contractAddr;
  currContract.setProvider( web3RpcProvider.currentProvider );
 ...
 *
 */
PriceOracle.prototype.getBaseCurrency = async function(contractAddress){
  currContract.options.address = contractAddress;
  currContract.setProvider( web3RpcProvider.currentProvider );
  const transactionObject = currContract.methods.baseCurrency();
  const encodedABI = transactionObject.encodeABI();
  const transactionOutputs = helper.getTransactionOutputs( transactionObject );
  const response = await helper.call(web3RpcProvider, contractAddress, encodedABI, {}, transactionOutputs);
  logger.info("response");
  logger.info(response[0]);
  return Promise.resolve(web3RpcProvider.utils.toUtf8(response[0]));
},
 ...
 *
 */
PriceOracle.prototype.getQuoteCurrency = async function(contractAddress){
  currContract.options.address = contractAddress;
  currContract.setProvider( web3RpcProvider.currentProvider );
