...
// Declare variables.
const priceOracleContractName = contractNameConstants.priceOracle;
...
class DeployHelper {
   ...
   */
  async deployAndSetInOps(web3, deployerAddress, opsAddress, adminAddress, baseCurrency, quoteCurrency, txOptions) {
    const oThis = this;
    ...
    await oThis.setAdminAddress(web3, adminAddress, contractAddress, txOptions);
  ...
  deployOpsContract(web3, deployerAddress, baseCurrency, quoteCurrency, txOptions) {
    ...
    const tx = oThis.deployRawTx(web3, deployerAddress, baseCurrency, quoteCurrency, txOptions);
    let txReceipt = {};
  ...
  deployRawTx(web3, deployerAddress, baseCurrency, quoteCurrency, txOptions) {
    ...
    const abiBinProvider = oThis.abiBinProvider,
      abi = abiBinProvider.getABI(priceOracleContractName),
      bin = abiBinProvider.getBIN(priceOracleContractName),
    ...
    const options = {
      ...
      data: abiBinProvider.getBIN(priceOracleContractName)
    };
   ...
   * @param {string/number} txOptions.chainId
   *
   * @returns {PromiseLike<{contractAddress: *}> | Promise<{contractAddress: *}> | *}
   */
  setOpsAddress(web3, opsAddress, contractAddress, txOptions) {
    const oThis = this;

    const tx = oThis.setOpsAddressTx(web3, opsAddress, contractAddress, txOptions);
  ...
  setAdminAddress(web3, adminAddress, contractAddress, txOptions) {
    ...
    const tx = oThis.setAdminAddressTx(web3, adminAddress, contractAddress, txOptions);
